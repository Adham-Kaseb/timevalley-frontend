...
export default function EditConsultationPage({
  ...
  const [tagsInput, setTagsInput] = useState("Strategy, Venture Builder");
  ...
  useEffect(() => {
    async function loadCard() {
      setLoading(true);
      try {
        const all = await consultationsService.getAllAdmin();
        ...
        if (found) {
          ...
          setConsultantName(found.consultantName || "Dr. Wael");
          setConsultantTitle(found.consultantTitle || "Founder & Managing Partner");
          setConsultantAvatar(found.consultantAvatar || "/images/team/CEO.jpg");
  ...
  const handleUpdate = async (publishedState: boolean) => {
    ...
    setSubmitting(true);
    const payload: Partial<CreateConsultationPayload> = {
      title,
      category,
      duration,
      price: Number(price),
    ...
    try {
      await consultationsService.updateConsultation(id, payload);
      router.push("/admin/consultations");
    } catch (err) {
      console.error("Failed to update consultation card", err);
      alert("Failed to update consultation card. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error("Failed to delete consultation card", err);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-200 pb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 font-semibold mt-1">
            Modify card parameters, pricing, and description with real-time live preview.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-red-50 hover:bg-red-600 text-red-600 hover:text-white font-bold text-xs transition-colors cursor-pointer"
          >
          ...
          >
            {submitting ? "Saving..." : "Update Consultation Card"}
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-7 bg-white border border-gray-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
            ...
            <label htmlFor="isPublished" className="text-xs font-bold text-gray-800">
              Card is Published & Visible on /consultations Page
            </label>
          </div>
        </div>
